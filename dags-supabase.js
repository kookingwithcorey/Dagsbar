...
(function () {
  ...
  async function saveWhiskeyIQLog(score) {
    ...
    return await db.from("whiskey_iq_logs").insert({
      user_id: user.id,
  ...
  async function saveWhiskeyLog(entry) {
    const db = getClient(); const user = await requireLogin(); if (!db || !user) return { error: "Not logged in" };
    return await db.from("whiskey_logs").insert({
      user_id: user.id,
  ...
  async function saveBlindTasting(tasting, bottles) {
    ...
    const childRows = (bottles || []).map((bottle, index) => ({
      tasting_id: parent.id, user_id: user.id, bottle_id: bottle.bottle_id || null,
      label: bottle.label || String.fromCharCode(65 + index), revealed_name: bottle.revealed_name || bottle.name || null,
      nose_score: bottle.nose_score ?? bottle.nose ?? null, palate_score: bottle.palate_score ?? bottle.palate ?? null,
      finish_score: bottle.finish_score ?? bottle.finish ?? null, total_score: bottle.total_score ?? bottle.total ?? null,
      ranking: bottle.ranking || null, notes: bottle.notes || null
    }));
    if (childRows.length) { const { error: childError } = await db.from("blind_tasting_bottles").insert(childRows); if (childError) return { data: parent, error: childError }; }
  ...
  function updateWhiskeyIQNav() {
    ...
    [
      ...
      if (exists) return;
      const a = document.createElement("a"); a.href = link.href; a.textContent = link.text;
      if (link.external) { a.target = "_blank"; a.rel = "noopener noreferrer"; }
      const legacy = nav.querySelector(".legacy-iq-btn"); legacy ? nav.insertBefore(a, legacy) : nav.appendChild(a);
    });
  ...
  function syncSharedControlsVisibility() { const control = document.getElementById("dagsAccountControl"); if (control) control.classList.toggle("dags-login-hidden", shouldHideSharedControls()); }
  ...
  function getCurrentTheme() { return document.documentElement.getAttribute("data-theme") || localStorage.getItem(THEME_KEY) || "dark"; }
  function sunIcon() { return `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="4.5"></circle><path d="M12 2.5v2.2M12 19.3v2.2M4.22 4.22l1.56 1.56M18.22 18.22l1.56 1.56M2.5 12h2.2M19.3 12h2.2M4.22 19.78l1.56-1.56M18.22 5.78l1.56-1.56"></path></svg>`; }
  function moonIcon() { return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21 12.8A8.5 8.5 0 1 1 11.2 3a6.7 6.7 0 0 0 9.8 9.8Z"></path></svg>`; }
  ...
  function updateThemeButton() { setThemeButtonIcon(document.getElementById("dagsThemeControl")); }
  function applyTheme(theme) {
    const next = theme === "light" ? "light" : "dark";
    document.documentElement.setAttribute("data-theme", next);
    localStorage.setItem(THEME_KEY, next); localStorage.setItem("dags-home-theme", next); localStorage.setItem("dags-theme", next);
    updateThemeButton();
  }
  ...
  function getOrCreateThemeButton() {
    ...
    btn.id = "dagsThemeControl"; btn.classList.add("dags-theme-control"); btn.type = "button";
  ...
  function injectFloatingAccountControl() {
    ...
    style.textContent = `
      .dags-account-control{position:fixed;top:28px;right:28px;z-index:9998;display:flex;align-items:center;justify-content:flex-end;gap:8px}.dags-account-control.dags-login-hidden,body.modal-open .dags-account-control{display:none!important}
